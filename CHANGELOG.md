...
### Fixed — 2026-08-19 — SEO regression: restore 301s for the ultra-high-brightness window-display range (#1 → #3 recovery)

**Root cause of the organic/lead collapse, found.** The rebuild brief required preserving URLs; instead new pages were built at NEW slugs and the OLD ranking URLs were left to **404**. Confirmed via Wayback + live testing: **80 of 383 (21%) old commercial `/product/*` URLs return 404 today** — Google's accumulated ranking equity on them evaporated. Template case: IDI dropped #1 → #3 for "ultra high brightness window display in ireland". The old #1 lived at `/product/ultra-bright-window-display-tni` (+ per-size `/product/{49,55,65}-3000-high-brightness-window-facing-display`, double-sided variants, `/high-bright-window-displays`) — **all 404**. The existing `androidConsolidation` map had even guessed the wrong legacy slug (`ultra-high-bright-display-tni`, which never existed) and missed the real one. Added `windowDisplayRedirects` in `resolveRedirect` — 301s every real legacy window/high-brightness URL (±trailing slash) to the new range page `/product/ultra-high-bright-display` (which carries all 43–75" sizes + double-sided). This consolidates the lost equity back onto the ranking page. First of a broader redirect-restoration + on-page parity audit across all money terms (digital signage, menu screens/boards, advertising screens, touch/kiosks, LED walls). Deploy needed (avclaudeadmin).

### Changed — 2026-08-19 — Menu enlarged + Screen Solutions mega given its second level back

Gerry: the injected menus were "too small" and "missing the second level". Enlarged the whole header (bar 64→72px, logo 40→46px, nav 14→15.5px/semibold, dropdowns 14→15px, CTA taller). Rebuilt the Screen Solutions mega from a flat 10-image-tile grid into a **two-level column mega** matching the old WP tabbed mega (Display/Touchscreen/Outdoor/High-Brightness/Interactive/Self-Ordering/LED): 4 category-heading columns (level 1 → category page) each listing its products (level 2), + a featured image tile. Hardened the hover-reveal + submenu display with `!important` so legacy WP CSS can't suppress them. Verified locally (columns render, old tiles gone, 72px bar). Deploy needed.

### Changed — 2026-08-19 — One modern header + mega menu + footer across the WHOLE site

The migration had only moved ~7 URLs to React (which render `<Nav>`/`<Footer>`); every other page — including the homepage — still shipped the old Elementor header/footer, so the site looked like **two different sites** with two different mega menus (Gerry's #1 complaint). `rewriteMirrorHtml` now injects a **self-contained HTML+CSS clone of the React `<Nav>`/`<Footer>`** (namespaced `.idi-mm`/`.idi-mf`, pure-CSS hover menus, one tiny inline script for the mobile hamburger) into every mirror page and **hides the legacy Elementor header (id 229) + footer (id 267)** via one `display:none` rule. Result: identical modern header, Screen Solutions image-tile mega menu, dropdowns and navy footer on React and mirror pages alike. Colours/paths match the React components exactly (brand-navy #002B7A; logo/tile assets already served from `dist/client`). MENU kept in sync with `src/components/nav/Nav.tsx` + `Footer.tsx`. Validated locally against the real mirror homepage (old chrome hidden, 10 tiles, correct placement). Also this session: Google Ads lead conversion now fires on mirror Elementor form submits (`buildAdsLeadConversionSnippet`, commit 22c2012). Deploy needed (avclaudeadmin).

### Changed — 2026-08-18 — Visitor Assist: public-service angle (qfusion.ie) + tidy-up

Follow-up to the QFusion rebrand. `rewriteMirrorHtml` now: (1) leans into the **qfusion.ie** public-service positioning — the VA CTA is a "QFusion — built for Ireland's public services" block (hospitals, county councils, motor-tax, social-welfare; bilingual; cloud-based) linking **qfusion.ie** + app.qfusion.ai; (2) retires the ESII-only bits — "Events Management" → "SMS Notifications" (QFusion does SMS, not events), and drops the reception video-call / badge-printing claims; (3) **strips the Customer Counting + Interactive Vending tiles** from the WP/Elementor VA mega-menu (same div-neutral wpr-promo-box removal as the mirror products; both still 301 → /queue-management-system/). Verified div-neutral on the mirror pages. Deploy needed (avclaudeadmin).
...
The UHB Window Displays range (MOY-43/49/55/65/75UHBHD) is now a React MDX product page — the first "range-page kit" from the Product Bible website workstream (benchmark: Allsee's range pages). Per-size variant spec tables for all five sizes (specs verified against the OEM sheets in Dropbox `Supplier_Spec_Sheets`, dimensions cross-checked against the technical drawings), 8 downloads (range brochure + 4 per-size spec sheets + 3 technical drawings, copied de-branded into `/public/brochures/`), FAQs, and copy leading on 3,000 cd/m² sunlight readability + TNI 110°C blackening resistance + ScreenFusion upgrade path. **Spec correction found while building: panels are 3,000 cd/m² per the OEM sheets, not the 3,500 in the Moytronix price file header — flagged back to the Product Bible data-fixes list.** URL taken over via MIRROR_EXCLUDE (+ trailing-slash 301, sitemap handled by the products loader). The separate TNI mirror page stays for now.

### Fixed — 2026-08-18 — Mobile menu panel + header CTA clipping (from Gerry's phone screenshots)

The snapshot froze the Elementor n-menu in its desktop layout, so on phones the opened hamburger menu rendered as a cramped right-aligned strip of tiny links overlapping the logo — the site's worst mobile defect. `rewriteMirrorHtml` step 1a⁷ injects `#idi-mobile-fixes` CSS on every mirror page: the OPEN menu (targeted via `:has(.e-n-menu-toggle[aria-expanded="true"])` — the nested-menu JS still maintains that attribute) becomes a full-width dark panel under the header with stacked 17px tap targets, per-item dividers, scrollable mega-menu content. Also compacts the header "Get In Touch" button that clipped off the right viewport edge (shared header template, data-id 36f8b19) and force-hides `.elementor-background-video-container` on phones (stray playback artefacts). Closed-state behaviour untouched; browsers without `:has` degrade to prior behaviour.

### Fixed — 2026-08-18 — Restore Elementor lazy-loaded chunks the wget snapshot missed (lightbox, gallery + 100 more)

wget can't capture webpack chunks that only load on user interaction, so the mirror was missing most of Elementor's lazy bundles — **user-visible breakage**: the image **lightbox** (chunk 216) and Pro **gallery** (chunk 543) failed with ChunkLoadError, i.e. clicking gallery images on mirror pages did nothing. Restored the complete chunk sets into the mirror: 64 core chunks extracted from the official `elementor.3.35.4.zip` (downloads.wordpress.org — same release the site ran, content hashes match exactly, `lightbox.d1799e50…` + `nested-title-keyboard-handler.2a67d3cc…` verified), and 40 Pro chunks (incl. `gallery.06be1c07…`) copied from the retired WordPress install still on disk at `/var/www/vhosts/interactivedisplays.ie/httpdocs/` — same origin as the snapshot itself. ~2 MB total. Discovery: that httpdocs tree is the complete old WP site — the "lost WordPress login" is moot (files on disk, nothing serves them); it's also a recovery source for any other assets the snapshot missed.
