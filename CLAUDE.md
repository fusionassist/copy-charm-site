...
**Runtime precedence — important when adding new routes:** Both `bun dev` and `bun run start` serve the wp-mirror first and fall through to TanStack only if the URL doesn't exist in the mirror. This is intentional: it gives "rebuild on top" semantics while the new routes are being built — the live site shows the legacy WP content immediately, and each new TanStack route takes over its URL as it lands. **Consequence:** when you add a real route in `src/routes/` whose path collides with a mirror URL (e.g. `src/routes/contact-us.tsx` and `wp-mirror/contact-us/index.html` both exist), the mirror will win. To make the TanStack route win, add the path to the `MIRROR_EXCLUDE` list in `start-node.mjs` (and the `serveMirror` plugin in `vite.config.ts` for dev parity).
